...
public partial class ColdFusionParser
{
    ...
    // Extract a named attribute value from a tag string (cached per attribute name)
    private static readonly ConcurrentDictionary<string, Regex> TagAttributeRegexCache = new();

    private static Regex TagAttributeRegex(string attrName) =>
        TagAttributeRegexCache.GetOrAdd(attrName, name =>
            new Regex($@"\b{name}\s*=\s*[""'](?<value>[^""']+)[""']",
