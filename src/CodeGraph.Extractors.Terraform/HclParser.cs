...
namespace CodeGraph.Extractors.Terraform;
...
public partial class HclParser
{
    ...
    private static readonly ConcurrentDictionary<string, Regex> StringValueRegexCache = new();
    private static readonly ConcurrentDictionary<string, Regex> MapKeyRegexCache = new();

    private static string? ExtractStringValue(string body, string key)
    {
        // Match: key = "value" or key = value
        var regex = StringValueRegexCache.GetOrAdd(key, k =>
            new Regex($@"(?:^|\n)\s*{Regex.Escape(k)}\s*=\s*""?(?<value>[^""\n]+?)""?\s*(?:\n|$)",
    ...
    private static Dictionary<string, string> ExtractMapValues(string body, string key)
    {
        ...
        var regex = MapKeyRegexCache.GetOrAdd(key, k =>
            new Regex($@"{Regex.Escape(k)}\s*=\s*\{{", RegexOptions.Compiled));
        var match = regex.Match(body);
        if (!match.Success) return result;

        var mapBody = ExtractBlockBody(body, match.Index + match.Length - 1);
        foreach (Match entry in MapEntryRegex().Matches(mapBody))
        {
