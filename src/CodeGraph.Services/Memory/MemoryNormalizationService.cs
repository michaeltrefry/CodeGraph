...
public partial class MemoryNormalizationService
{
    ...
    public async Task<StoreMemoryResult> NormalizeAndUpsertAsync(MemoryExtractionResult extraction, string source)
    {
        ...
        for (var i = 0; i < extraction.Nodes.Count; i++)
        {
            ...
            var candidates = await _store.FindCandidateEntityIdsAsync(normalizedId);
        ...
        await _store.AddRelationshipsBatchAsync(relationships);
