        ...
        CancellationToken ct = default)
    {
        if (!string.IsNullOrWhiteSpace(localPath) && Directory.Exists(localPath))
        {
            if (!string.IsNullOrWhiteSpace(repoUrl))
                throw new InvalidOperationException("GitLab repository resolution cannot combine a local path with a remote URL.");
            ...
            return new ResolvedRepository(
                canonicalPath,
                $"gitlab-path:{canonicalPath.Replace('\\', '/')}",
                null,
                null);
        }
        ...
        if (string.IsNullOrWhiteSpace(repoUrl))
            throw new InvalidOperationException($"Unable to resolve GitLab repository URL for '{repoName}'.");
        var (canonicalIdentity, sourceGroup) = RepositoryIdentity.FromRemote("gitlab", repoName, repoUrl);
