...
public class ProjectReviewBackgroundRunner(
    IServiceScopeFactory scopeFactory,
    ILogger<ProjectReviewBackgroundRunner> logger) : IProjectReviewBackgroundRunner
{
            ...
            _ = runner.ContinueWith(t =>
            {
                if (t.IsFaulted)
                {
                    logger.LogError(t.Exception,
                        "Project review background runner crashed for review run {ReviewRunId}",
                        reviewRunId);
                }
    ...
    private async Task RunInBackgroundAsync(long reviewRunId)
    {
        ...
        var service = scope.ServiceProvider.GetRequiredService<IProjectReviewService>();
