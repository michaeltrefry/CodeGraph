...
public class CodeGraphSyntaxWalker : CSharpSyntaxWalker
{
    ...
    {
        // Detect: bus.Publish(someEvent) or _bus.Publish(new SomeEvent { ... })
    ...
    /// <summary>
    /// Extract routing metadata from MassTransit endpoint attributes on event/command classes.
    /// Creates Queue and Exchange nodes and ROUTED_TO/BOUND_TO edges.
    /// Recognizes [EntityName], [ExchangeName], and [ConfigureConsumeTopology] attributes.
    /// </summary>
        ...
        ClassDeclarationSyntax node)
    {
        foreach (var attr in classSymbol.GetAttributes())
        {
            ...
            if (attrName is not ("EntityName" or "EntityNameAttribute"
                or "ExchangeName" or "ExchangeNameAttribute"))
                continue;
            ...
            if (classNode is not null)
            {
                ...
                classNode.Properties["is_message_contract"] = true;
            }
    ...
    /// or has a MassTransit endpoint attribute.
    ...
    private void DetectEventMessageFields(INamedTypeSymbol classSymbol, string classQN)
    {
        ...
        var hasEventAttribute = classSymbol.GetAttributes().Any(a =>
            a.AttributeClass?.Name is "EntityName" or "EntityNameAttribute"
                or "ExchangeName" or "ExchangeNameAttribute");
        ...
        InvocationExpressionSyntax node, string methodName, string callerQN)
    {
        ...
        // Get the receiver identifier name (e.g., _bus, _httpClient)
        ...
        // MassTransit bus pattern: _bus.Publish, _publishEndpoint.Publish, etc.
        if (receiverLower.Contains("bus") || receiverLower.Contains("publish"))
        {
            if (methodName is "Publish" or "Send")
            {
    ...
    /// MassTransit: AddConsumer&lt;T&gt;(), AddConsumers(assembly), cfg.ReceiveEndpoint("queue", e => e.Consumer&lt;T&gt;())
        ...
        IMethodSymbol method, string callerQN)
    {
        ...
        // Pattern 2: AddConsumers(typeof(X).Assembly) — MassTransit assembly scanning
