...
public class ProjectService(ITcConfiguration<CodeGraphServiceSettings> settings,
    ...
    ILogger<ProjectService> logger) : IProjectService
{
    ...
    private SemaphoreSlim RepoSemaphore
    {
        get
        {
            var max = settings.Current.IndexingOptions.MaxParallelRepos;
    ...
    private async Task ProcessRepositoryCore(ProcessRepository message, CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(message.GitLabGroup) && !string.IsNullOrWhiteSpace(gitLabUrl))
        {
            ...
            var pathWithNamespace = gitLabUrl.Replace(settings.Current.GitLabOptions.BaseUrl, "");
        ...
        if (message.ShouldIndex)
        {
            ...
            await serviceBus.Publish(new RepositoryIndexingCompleted
            {
                Name = message.Name,
                RepoPath = repoPath,
                GitLabUrl = gitLabUrl,
