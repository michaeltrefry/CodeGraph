...
public class Neo4jMemoryGraphStore : IMemoryGraphStore
{
    ...
    public async Task UpsertEntitiesBatchAsync(IReadOnlyList<MemoryEntity> entities)
    {
        ...
        await session.ExecuteWriteAsync(async tx =>
        {
            await tx.RunAsync(
                ...
                MERGE (e:MemoryEntity {username: item.username, id: item.id})
                ON CREATE SET
                    e.label = item.label,
                    e.type = item.type,
    ...
    public async Task AddRelationshipsBatchAsync(IReadOnlyList<MemoryRelationship> relationships, string username)
    {
        ...
        await session.ExecuteWriteAsync(async tx =>
        {
            await tx.RunAsync(
                """
                UNWIND $batch AS item
                MATCH (a:MemoryEntity {username: $username, id: item.fromId})
                MATCH (b:MemoryEntity {username: $username, id: item.toId})
                CREATE (a)-[:RELATES_TO {
    ...
    public async Task CreateObservationAsync(MemoryObservation obs, string? fromEntityId = null, string? toEntityId = null)
    {
        ...
        await session.ExecuteWriteAsync(async tx =>
        {
            await tx.RunAsync(
                ...
                WITH o
                OPTIONAL MATCH (e1:MemoryEntity {username: $username, id: $fromEntityId})
                OPTIONAL MATCH (e2:MemoryEntity {username: $username, id: $toEntityId})
    ...
    public async Task<List<(MemoryEntity Entity, double Score)>> VectorSearchAsync(float[] queryEmbedding, string username, int topK = 5)
    {
        int[] overfetchFactors = [5, 20, 100];
        foreach (var factor in overfetchFactors)
        {
            var results = await VectorSearchWithOverfetchAsync(queryEmbedding, username, topK, topK * factor);
    ...
    private async Task<List<(MemoryEntity Entity, double Score)>> VectorSearchWithOverfetchAsync(
        float[] queryEmbedding, string username, int topK, int fetchSize)
    {
        ...
        try
        {
            var result = await session.RunAsync(
                ...
                LIMIT $topK
                """,
                new { fetchSize, topK, embedding = queryEmbedding, username });
    ...
    public async Task<List<MemoryEntity>> TextSearchAsync(string query, string username, int limit = 5)
    {
        await using var session = _sessionFactory.GetSession(AccessMode.Read);
        ...
        var result = await session.RunAsync(
            ...
            LIMIT $limit
            """,
            new { query = escapedQuery, username, limit });
    ...
    public async Task<List<MemoryRelationshipDetail>> GetRelationshipsAsync(string entityId, string username)
    {
        await using var session = _sessionFactory.GetSession(AccessMode.Read);
        var results = new List<MemoryRelationshipDetail>();

        var cypher =
            "MATCH (e:MemoryEntity {username: $username, id: $entityId})-[rel:RELATES_TO]-(other:MemoryEntity {username: $username}) " +
        ...
        var result = await session.RunAsync(cypher, new { entityId, username });
        ...
        IReadOnlyList<string> seedIds, string username, int hops = 2, int maxNodes = 20, int maxRelsPerPair = 5)
    {
        ...
        var entityCypher =
            ...
            "MATCH (seed:MemoryEntity {username: $username, id: seedId}) " +
            $"OPTIONAL MATCH (seed)-[:RELATES_TO*1..{clampedHops}]-(other:MemoryEntity {{username: $username}}) " +
        ...
        var entityResult = await session.RunAsync(entityCypher, new { seedIds, username, maxNodes });
        ...
        var relResult = await session.RunAsync(
            """
            MATCH (a:MemoryEntity {username: $username})-[rel:RELATES_TO]->(b:MemoryEntity {username: $username})
            ...
            ORDER BY rel.timestamp DESC
            """,
            new { username, nodeIds, maxRelsPerPair });
    ...
    public async Task<List<MemoryObservation>> GetUnresolvedObservationsForEntitiesAsync(IEnumerable<string> entityIds, string username)
    {
        await using var session = _sessionFactory.GetSession(AccessMode.Read);
        ...
        var result = await session.RunAsync(
            """
            CALL {
                MATCH (o:MemoryObservation {username: $username, resolved: false})-[:OBSERVES]->(e:MemoryEntity)
                WHERE e.id IN $entityIds
                RETURN DISTINCT o
            UNION
                MATCH (o:MemoryObservation {username: $username, resolved: false})
            ...
            RETURN o.id AS id, o.claim AS claim, o.conflictsWith AS conflictsWith,
                   o.source AS source, o.username AS username, o.timestamp AS timestamp,
                   o.resolved AS resolved, o.resolution AS resolution,
                   o.resolvedByMemoryId AS resolvedByMemoryId
            ORDER BY o.timestamp DESC
            """,
            new { entityIds = entityIds.ToList(), username });
    ...
    public async Task<MemoryGraphSnapshot> GetFullGraphAsync(string username, int limit = 200, int skip = 0)
    {
        ...
        var entityResult = await session.RunAsync(
            """
            MATCH (e:MemoryEntity {username: $username})
        ...
        var relResult = await session.RunAsync(
            """
            MATCH (a:MemoryEntity {username: $username})-[r:RELATES_TO]->(b:MemoryEntity {username: $username})
            WHERE a.id IN $nodeIds AND b.id IN $nodeIds
            RETURN a.id AS source, b.id AS target,
                   r.relationship AS relationship, r.context AS context,
                   r.timestamp AS timestamp
            """,
            new { username, nodeIds });
        ...
        var countResult = await session.RunAsync(
            "MATCH (e:MemoryEntity {username: $username}) RETURN count(e) AS total",
            new { username });
    ...
    public async Task<List<string>> FindCandidateEntityIdsAsync(string candidateId, string username, int limit = 20)
    {
        await using var session = _sessionFactory.GetSession(AccessMode.Read);
        ...
        var result = await session.RunAsync(
            ...
            LIMIT $limit
            """,
            new { query = escapedTerms, username, limit });
    ...
    public async Task<MemoryEntity?> GetEntityAsync(string entityId, string username)
    {
        await using var session = _sessionFactory.GetSession(AccessMode.Read);
        var result = await session.RunAsync(
            """
            MATCH (e:MemoryEntity {username: $username, id: $id})
            RETURN e.id AS id, e.label AS label, e.type AS type,
                   e.summary AS summary, e.source AS source,
                   e.username AS username,
                   e.createdAt AS createdAt, e.updatedAt AS updatedAt
            """,
            new { id = entityId, username });
