...
public class ProjectQueryService(IGraphStore store, GitLabOptions gitLabOptions) : IProjectQueryService
{
    ...
    {
        var filePath = await RepoFileResolver.ResolveAsync(name, "README.md", gitLabOptions, store);
        if (filePath is null)
            return null;
    ...
    internal static ProjectListItem MapProjectListItem(ProjectInfo p) =>
        new(p.Name, p.RepoUrl, p.GitLabGroup, p.LocalPath, p.LastCommitSha, p.IndexedAt,
