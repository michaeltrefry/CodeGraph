...
public class MemoryRetrievalService
{
    ...
    public async Task<MemoryQueryResult> QueryAsync(string topic, string username, int hops = 2, int maxNodes = 5, int maxRelsPerEntity = 8)
    {
        _logger.LogInformation("Querying memory for topic: {Topic}, user: {Username}", topic, username);

        var (seedEntities, queryEmbedding) = await FindSeedEntitiesAsync(topic, username, maxNodes);
        ...
        var (entities, relationships) = await _store.GetSubgraphAsync(seedIds, username, hops, maxNodes);
        ...
        var conflicts = await _store.GetUnresolvedObservationsForEntitiesAsync(allEntityIds, username);
    ...
    private async Task<(List<(MemoryEntity Entity, double Score)> Entities, float[]? Embedding)> FindSeedEntitiesAsync(
        string topic, string username, int maxNodes)
    {
        if (_embedding.IsAvailable)
        {
            try
            {
                ...
                {
                    var vectorResults = await _store.VectorSearchAsync(embedding, username, maxNodes);
        ...
        var textResults = await _store.TextSearchAsync(topic, username, maxNodes);
